...
function validate()
	...
	if (document.add_new_debit_cards.name.value == '')
          {
		alert('Name : Please Enter debit card name');
		document.add_new_debit_cards.name.focus();
		return false;
          }
	
	if (document.add_new_debit_cards.bname.value == '')
          {
		alert('Name : Please Enter bank name');
		document.add_new_debit_cards.bname.focus();
		return false;
          }
	if (document.add_new_debit_cards.image1.value== '')
          {
		alert('Image : Please Select an image');
		document.add_new_debit_cards.image1.focus();
		return false;
          }	
...
if(isset($_POST['submit']))
{
  ...
  $path = "../db_card_images/".$file;
  $date_added = date("Y/m/d");
//echo $name;

  $sql = "INSERT INTO farmhouse_debit_card_details (card_name,bank_name,image,date_added) 
  values('$cname','$bname','$image','$date_added')";
  
  if(mysqli_query($conn,$sql))
				{
					echo "values added successfully";
					if (copy($_FILES["image1"]["tmp_name"], "../db_card_images/". $_FILES["image1"]["name"]))
                        {
...
?>
  ...
  <body> 
    <form method="POST" name='add_new_debit_cards' action = "<?php $PHP_SELF ?>" enctype="multipart/form-data" onsubmit = "return validate()">
    ...
    <div class="container">
         ...
         <div class="row  pad-top">
                ...
                <div class="col-md-4 col-md-offset-4 col-sm-6 col-sm-offset-3 col-xs-10 col-xs-offset-1">
                        ...
                        <strong>  <h4> Add New Debit Cards </h4></strong>  
                            </div>
...
<br/>
                                         ...
                                         <div class="form-group input-group">
                                            ...
                                            <input type="text" name= "cname" id  = "cname" class="form-control" maxlength="50" placeholder="Debit Card Name" />
                                        </div>
